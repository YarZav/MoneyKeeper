...
extension CashFlowDAO: CashFlowDAOProtocol {
    func getModels() -> [CashFlowCoreDataModel]? {
//        let userCoreDataModel = coreDataManager.getCoreDataModels(entityName: "User")?.first
...
//        return cashFlowCareDataModels
    ...
    func deleteModel(_ model: CashFlowCoreDataModel, callback: @escaping (Error?) -> Void) {
//        let cashFlowCoreDataModels = getCashFlowCoreDataModels(for: userCoreDataModel)
//        if let model = cashFlowCoreDataModels?.first(where: { $0.identifier == id }) {
